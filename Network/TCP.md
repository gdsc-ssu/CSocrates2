...
Unix는 모든 것이 file이라는 개념이 있기 때문에 소켓또한 File Descriptors라 하는 것이다. 
소켓은 복잡한 TCP/IP 의 내부 동작(패킷 분할, 재전송 등)을 몰라도, 마치 파일에 글을 쓰고 읽듯이 네트워크 통신을 할 수 있게 해주는 인터페이스 역할을 한다. 

**소켓을 식별하는 5요소**
네트워크에 수많은 소켓이 동시에 존재할 때, 커널은 어떤 패킷이 어떤 소켓으로 가야하는지 아래 5가지 정보를 바탕으로 찾는다.

1. Protocol : TCP인지 UDP인지 
2. Souce IP : 내 컴퓨터의 주소
3. Source Port : 내 프로그램의 포트 번호
4. Destination IP : 상대방 컴퓨터 주소
5. Destination Port : 상대방 프로그램의 포트 번호 

이중에 하나라도 다르면 다른 소켓으로 간주된다. 
![[스크린샷 2026-01-26 오후 11.09.28.png]]

#### 서버 측 소켓

1. **소켓 생성:** 클라이언트의 연결 요청을 대기하기 위한 소켓 생성
2. **Binding**(주소 할당): IP 주소와 Port 번호를 할당받는다.
3. **Listening**: 연결되지 않은 소켓을 대기모드로 전환한다.
4. **accept()**: 클라이언트가 연결 요청을 하면, 수락 후 실질적인 통신을 위한 소켓을 생성한다.   
    (3번의 소켓은 only 대기용)
5. **데이터 송수신**: 연결이 설정되면, 클라이언트와 서버가 소켓을 통해 데이터를 주고받는다.
6. **연결 종료**: 소켓을 닫는다.

#### 클라이언트 측 소켓

1. **소켓 생성**: 서버에 연결하기 위한 소켓을 생성
2. **서버에 연결**: 서버가 설정한 IP, Port로 연결을 시도한다.
3. 서버가 accept()로 클라이언트의 요청을 수락하면 클라이언트의 socket descriptor가 반환되고 **데이터를 송수신할 준비**가 된다.
4. **데이터 송수신**: 데이터를 주고받는다.
5. **연결 종료**: 소켓을 닫는다.


**소켓이 동작하는 방식에는 2가지가 있다.** 

- 리스닝 소켓 : 서버에서 문지기 역할을 한다. 연결요청이 오면 새로운 소켓을 만들어 실제 통신을 한다. 
- 커넥티드 소켓 : 실제 데이터를 주고받는 소켓이다. 서버는 클라이언트마다 별도의 커넥티드 소켓을 생성하여 1:1 통신을 한다. accept()함수가 반환하는 소켓이다
