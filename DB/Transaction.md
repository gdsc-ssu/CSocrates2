...
중간에 오류가 나면? 출금만 되고 입금이 안 되면 돈이 사라진다 트랜잭션은 이 두 연산을 하나로 묶어 일관성을 보장한다

> [!info] INFO 트랜잭션의 목표 : **데이터 무결성을 지키는 것!**
...
트랜잭션에 포함된 작업은 **하나의 단위처럼** 처리됨 즉, 모든 작업이 성공하면 **COMMIT**, 하나라도 실패하면 **ROLLBACK**

- 예: 송금 시 출금은 성공했지만 입금이 실패한 경우 → 전체 작업을 되돌려야 정상
...
- 예: 재고를 -1 감소시켰는데 주문 생성이 실패한 경우 → 데이터가 규칙을 어기게 되며, 트랜잭션은 이를 방지
...
여러 트랜잭션이 동시에 실행되더라도 **서로 간섭하지 않아야** 함 중간 결과가 노출되면 잘못된 데이터가 저장될 수 있음

- 예: 호텔 101호를 동시에 두 사용자가 예약한 상황 → 격리성이 충분하지 않다면 두 사람 모두 예약에 성공하는 문제가 발생
...
트랜잭션이 커밋된 이후에는 결과가 **영구적으로 저장**됨 서버가 장애를 일으키더라도 데이터는 유지되어야 함
...
SAVEPOINT는 복잡한 트랜잭션에서 부분적인 실패를 세밀하게 제어할 때 유용하다. 
단, 남용하면 트랜잭션 흐름이 복잡해져 오히려 유지보수가 어려워질 수 있다.
...
트랜잭션은 시작부터 종료까지 5가지 상태를 거친다. ![[Pasted image 20260323014825.png]]
...
격리성을 얼마나 엄격하게 적용할지를 단계별로 설정할 수 있다. 
격리 수준이 높을수록 정확성이 올라가지만 성능은 떨어진다.
...
이런 문제들을 일어나지 않게 만들 수 만들 수 있지만 그러면 제약사항이 많아져 동시에 처리 가능한 트랜젝션 수가 줄어들어 결국 DB의 전체 처리량이 하락하게 된다. -> 디비 성능 저하

일부 이상 현상을 허용하는 몇 가지 level을 만들어서 사용자가 필요에 따라 적절하게 선택할 수 있도록 하자
-> Isolation level
...
- **REPEATABLE READ** — 같은 트랜잭션 내에서 같은 데이터를 읽으면 항상 같은 값을 보장한다. MySQL InnoDB의 기본값이며, MVCC(Multi-Version Concurrency Control)를 통해 구현된다. InnoDB는 이 수준에서 Phantom Read도 **대부분 방지**한다 → 아래 Next-Key Lock 참고.
...
실제 실무에서는 이런 여러 isolation을 어떻게 정의하고 있을까 

#### MySql(innoDB)
![[Pasted image 20260324172713.png]]
표준 isolation과 동일

#### Oracle
![[Pasted image 20260324172747.png]]
READ UNCOMMITTED 사용안함
결국 READ COMMITTED와 SERIALIZABLE 두 가지만 사용

#### SQL server
![[Pasted image 20260324172814.png]]
snapshot 추가

#### postgreSQL
![[Pasted image 20260324172844.png]]
표준에서 정의한 이상현상 말고도 다른 이상현상을 정의함

---

## InnoDB 락 — Record / Gap / Next-Key Lock

> REPEATABLE READ에서 Phantom Read를 "대부분 방지"한다고 했는데, 그 구체적인 메커니즘이 Next-Key Lock이다.

### 핵심 전제: InnoDB는 인덱스에 락을 건다

InnoDB의 row-level lock은 **테이블 행이 아니라 인덱스 레코드**에 걸린다. 인덱스가 없으면 풀스캔이 발생하고, 모든 레코드에 락이 걸려 사실상 테이블 락이 된다.

> [!info] 왜 중요한가 
> SELECT FOR UPDATE 또는 UPDATE/DELETE를 실행할 때 인덱스가 없으면 예상치 못한 범위에 락이 걸린다. 성능 문제뿐 아니라 다른 쿼리가 전혀 관계없는 행에서도 대기하는 상황이 발생한다.

### 락의 세 가지 종류

#### 1. Record Lock

> "인덱스 레코드 하나에만 락을 거는 가장 기본적인 형태"

```sql
SELECT * FROM orders WHERE id = 20 FOR UPDATE;
-- id=20 레코드 하나만 잠금
-- id=15나 25 INSERT는 허용됨
```

- 등호 조건 + **존재하는** 레코드 + unique index 조합일 때 발생
- Gap은 잠그지 않으므로 새 행 삽입을 막지 못함

![[Pasted image 20260324162322.png]]

#### 2. Gap Lock

> "레코드 사이의 빈 구간(gap)에 락을 거는 것 — 레코드 자체는 잠그지 않는다"

```sql
SELECT * FROM orders WHERE id = 25 FOR UPDATE;
-- id=25는 존재하지 않음
-- (20, 30) 구간에 gap lock 획득
-- id=22나 id=28 INSERT 차단
-- 하지만 id=20 자체는 잠기지 않음
```

- 등호 조건 + **존재하지 않는** 레코드일 때 발생
- 레코드 삽입을 막기 위한 것이지, 기존 레코드 수정을 막는 것이 아님
- 표기법: `(20, 30)` — 양 끝 값을 포함하지 않는 열린 구간

![[Pasted image 20260324162340.png]]

#### 3. Next-Key Lock

> "Gap Lock + Record Lock의 합체 — InnoDB의 기본 락 방식"

```sql
SELECT * FROM orders WHERE id BETWEEN 20 AND 30 FOR UPDATE;
-- (10, 20] + (20, 30] 범위를 잠금
-- id=15, 25 INSERT 차단 (gap)
-- id=20, 30 수정도 차단 (record)
```

- 범위 조건(`BETWEEN`, `>`, `<`, `>=`, `<=`)에서 발생
- **Phantom Read를 막는 핵심 메커니즘**
- 표기법: `(10, 20]` — 왼쪽은 열리고 오른쪽(레코드)은 닫힌 반열린 구간

![[Pasted image 20260324162411.png]]


### Phantom Read 방지가 실제로 작동하는 방식

![[Pasted image 20260324162014.png]]

### 쿼리 패턴별 락 범위 정리

| 쿼리 패턴                        | 락 종류                | 잠기는 범위            | 새 행 삽입 차단  |
| ---------------------------- | ------------------- | ----------------- | ---------- |
| `WHERE id = 20` (존재, unique) | Record              | id=20만            | 없음         |
| `WHERE id = 25` (미존재)        | Gap                 | (20, 30)          | id∈(20,30) |
| `WHERE id BETWEEN 20 AND 30` | Next-Key            | (10,20] ∪ (20,30] | id∈(10,30) |
| `WHERE id > 30`              | Next-Key + supremum | (30,40] + (40,+∞) | id>30 전체   |
| 인덱스 없는 WHERE                 | Next-Key (전체)       | 전 범위              | 모든 삽입 차단   |

### Gap Lock이 걸리지 않는 예외 상황

| 상황                       | Gap lock | 이유                        |
| ------------------------ | -------- | ------------------------- |
| `READ COMMITTED` 격리 수준   | 없음       | RC는 Phantom Read 방지 의무 없음 |
| unique index + 등호 조건     | 없음       | 동일 키 삽입 자체가 불가            |
| non-unique index + 등호 조건 | 있음       | 같은 값 삽입 가능성 존재            |

> [!info] READ COMMITTED 주의 
> `READ COMMITTED`에서 `SELECT FOR UPDATE`를 사용하면 record lock만 걸리고 gap은 열려 있다. 범위 조회 후 새 행이 끼어드는 Phantom Read가 발생할 수 있으므로, 정합성이 중요한 쿼리에서는 격리 수준을 확인해야 한다.
...
- **재고 차감, 좌석 예약처럼 동시 접근이 치명적인 경우** → `SERIALIZABLE` 또는 비관적 락을 함께 고려한다.

```java
// READ_COMMITTED → gap lock 없음 → 범위 쿼리에서 phantom 가능
@Transactional(isolation = Isolation.READ_COMMITTED)
public List<Order> getOrdersInRange(int min, int max) { ... }

// REPEATABLE_READ (기본값) → next-key lock → phantom 방지
@Transactional(isolation = Isolation.REPEATABLE_READ)
public void processOrdersInRange(int min, int max) { ... }

// 재고 차감처럼 동시성이 critical한 경우: 비관적 락 명시
// → 내부적으로 SELECT ... FOR UPDATE 실행됨
@Lock(LockModeType.PESSIMISTIC_WRITE)
Optional<Product> findById(Long id);
```

> [!info] 핵심 트레이드오프 
> Gap lock 범위가 넓을수록 Phantom Read 안전성은 높아지지만 동시성은 낮아진다. `READ COMMITTED`는 동시성이 높지만 범위 쿼리에서 phantom 위험이 있다. 비즈니스 요구사항에 따라 격리 수준과 락 전략을 함께 결정해야 한다.
